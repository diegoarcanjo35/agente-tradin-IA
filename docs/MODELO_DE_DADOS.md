...
| `orders` | Ordem submetida (PAPER_LOCAL ou BYBIT_DEMO) | `idempotency_key` (único), `risk_evaluation_id` (FK), `status`, `exchange_order_id` |
| `executions` | Preenchimentos (fills), incluindo parciais | `order_id` (FK), `fill_qty`, `fill_price`, `fee`, `is_partial` |
| `positions` | Posições abertas/fechadas | `status`, `realized_pnl`, `fees_paid`, `opened_at`, `closed_at` |
| `account_snapshots` | Fotos periódicas de saldo/equity | `balance`, `equity`, `unrealized_pnl`, `mode` |
| `security_events` | Eventos de segurança (kill switch, bloqueio de endpoint de produção) | `event_type`, `detail` |
| `failures_reconciliations` | Falhas de execução e resultados de reconciliação | `kind` (FAILURE\|RECONCILIATION), `detail`, `resolved` |
| `system_state` | Estado vivo (linha única, id=1) | `trading_blocked`, `kill_switch_engaged`, `consecutive_losses`, `cooldown_until`, `api_failure_count` |
